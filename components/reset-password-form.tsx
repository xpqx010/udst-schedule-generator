...
export function ResetPasswordForm({ token }: { token: string }) {
  ...
  if (!token) return <div className="auth-form-shell"><header className="form-heading"><p>Account recovery</p><h2>Reset link missing</h2><span>This page needs the complete link from your reset email.</span></header><div className="form-alert" role="alert"><strong>Request a new link</strong><span>The reset token is missing.</span></div><p className="auth-switch"><Link href="/forgot-password">Request another reset link</Link></p></div>;

  return <div className="auth-form-shell">
    <header className="form-heading"><h2>Choose a new password</h2><span>Using a new password signs out any previous sessions.</span></header>
